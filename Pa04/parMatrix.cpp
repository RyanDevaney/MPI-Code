...
void rowShift(int** matrix, int numRows, int numCols, int numProcessors, int rank, int dest, int src);
void colShift(int** matrix, int numRows, int numCols, int numProcessors, int rank, int dest, int src);
...
int main(int argc, char* argv[])
   {
    ...
    rowShift( subMatrixA, setSize, setSize, numProcessors, myRank, dRow, sRow );
    colShift( subMatrixB, setSize, setSize, numProcessors, myRank, dCol, sCol);
...
void distributeMatrix(int** matrix, int** subMatrix, int numRows, int numCols,
   ...
   {
    int** temp;
    int subRow, subCol, index, innerIndex, processor, sendRow, sendCol;
    
    //calculate sub row/columns
    subRow = numRows / size;
    subCol = numCols / size;
    ...
    for( index = 0; index < subRow; index++ )
       {
        for( innerIndex = 0; innerIndex < subCol; innerIndex++ )
           {
            temp[ index ][ innerIndex ] = matrix[ index ][ innerIndex ];
           }
       }
    //calculate and send out rows/cols for each processor
    for( processor = 1; processor < numProcessors; processor++ )
       {
        ...
        for(int k = 0; index < sendRow + subRow; k++, index++ )
           {
            innerIndex = sendCol;

            for(int l = 0; innerIndex < sendCol + subCol; l++, innerIndex++ )
               {

                temp[k][l] = matrix[ index ][ innerIndex ];
               }
           }
        ...
        for( index = 0; index < subRow; index++ )
           {
            MPI_Send( temp[ index ], subCol, MPI_INT, processor, TAG, MPI_COMM_WORLD );
           }
       }

   }

//Performs row shifting based on cannon's algorithm
void rowShift(int** matrix, int numRows, int numCols, int numProcessors, int rank, int dest, int src)
   {
    int row, index;
    int** temp;
